# Serialization: Using explain() and diagnostics

Generated: 2025-08-10 13:46 UTC
...
+- Project [id#0L, cast((rand(7303529924190491823) * cast(100 as double)) as int) AS value#2, CASE WHEN ((id#0L % cast(2 as bigint)) = cast(0 as bigint)) THEN even ELSE odd END AS category#3]
...
Project [id#0L, (value#2 * 2) AS doubled#7]
+- Project [id#0L, cast((rand(7303529924190491823) * 100.0) as int) AS value#2]
   +- Range (0, 50000, step=1, splits=Some(11))

== Physical Plan ==
*(1) Project [id#0L, (value#2 * 2) AS doubled#7]
+- *(1) Project [id#0L, cast((rand(7303529924190491823) * 100.0) as int) AS value#2]
...
== Optimized Logical Plan ==
Aggregate [category#3], [category#3, avg(value#2) AS avg_value#14]
+- Project [cast((rand(7303529924190491823) * 100.0) as int) AS value#2, CASE WHEN ((id#0L % 2) = 0) THEN even ELSE odd END AS category#3]
   +- Range (0, 50000, step=1, splits=Some(11))
...
+- Exchange hashpartitioning(category#3, 200), ENSURE_REQUIREMENTS, [plan_id=29]
   +- *(1) HashAggregate(keys=[category#3], functions=[partial_avg(value#2)], output=[category#3, sum#19, count#20L])
      +- *(1) Project [cast((rand(7303529924190491823) * 100.0) as int) AS value#2, CASE WHEN ((id#0L % 2) = 0) THEN even ELSE odd END AS category#3]
         +- *(1) Range (0, 50000, step=1, splits=11)
...
+- Project [id#23L, cast((rand(3455651540520120074) * cast(100 as double)) as int) AS value#25, CASE WHEN ((id#23L % cast(2 as bigint)) = cast(0 as bigint)) THEN even ELSE odd END AS category#26]
   +- Range (0, 50000, step=1, splits=Some(11))
...
== Optimized Logical Plan ==
Project [id#23L, (value#25 * 2) AS doubled#30]
+- Project [id#23L, cast((rand(3455651540520120074) * 100.0) as int) AS value#25]
   +- Range (0, 50000, step=1, splits=Some(11))

== Physical Plan ==
*(1) Project [id#23L, (value#25 * 2) AS doubled#30]
+- *(1) Project [id#23L, cast((rand(3455651540520120074) * 100.0) as int) AS value#25]
   +- *(1) Range (0, 50000, step=1, splits=11)
...
🚀 1. Native Spark function:
   ...
   ✅ Processed 100,000 rows in 0.742s
== Physical Plan ==
* Project (3)
+- * Project (2)
   +- * Range (1)
...
Output [2]: [id#38L, cast((rand(8206742461253775309) * 100.0) as int) AS value#40]
Input [1]: [id#38L]
...
🐍 2. Python UDF:
   Execution plan with timing:
   ✅ Processed 100,000 rows in 0.070s
== Physical Plan ==
* Project (4)
+- BatchEvalPython (3)
   +- * Project (2)
...
+- Project [id#67L, value#69, category_python#75, (value#69 * value#69) AS value_squared#79]
   +- Project [id#67L, value#69, categorize_value(value#69)#74 AS category_python#75]
      +- Filter (value#69 > 10)
         +- Project [id#67L, cast((rand(1907857285726225586) * cast(100 as double)) as int) AS value#69, CASE WHEN ((id#67L % cast(2 as bigint)) = cast(0 as bigint)) THEN even ELSE odd END AS category#70]
...
+- Project [value#69, pythonUDF0#98 AS category_python#75, (value#69 * value#69) AS value_squared#79]
   +- BatchEvalPython [categorize_value(value#69)#74], [pythonUDF0#98]
      +- Filter (isnotnull(value#69) AND (value#69 > 10))
         +- Project [cast((rand(1907857285726225586) * 100.0) as int) AS value#69]
            +- Range (0, 80000, step=1, splits=Some(11))
      ...
      +- *(2) Project [value#69, pythonUDF0#98 AS category_python#75, (value#69 * value#69) AS value_squared#79]
         +- BatchEvalPython [categorize_value(value#69)#74], [pythonUDF0#98]
            +- *(1) Filter (isnotnull(value#69) AND (value#69 > 10))
               +- *(1) Project [cast((rand(1907857285726225586) * 100.0) as int) AS value#69]
                  +- *(1) Range (0, 80000, step=1, splits=11)


⏱️  Execution time: 1.100s

🔍 Results:
   Row(category_python='low', avg_value=17.413653534808418, avg_squared=319.31633387299877, count=11118)
   Row(category_python='high', avg_value=87.02277208201893, avg_squared=7624.9915220820185, count=20288)
   Row(category_python='medium', avg_value=49.414762299821504, avg_squared=2650.6711164743447, count=39777)
...
✅ Processed 150,000 rows in 0.072s
...
+- Project [id#131L, cast((rand(-4390728526079184670) * 100.0) as int) AS value#133], Statistics(sizeInBytes=976.6 KiB)
...
+- *(1) Project [id#131L, cast((rand(-4390728526079184670) * 100.0) as int) AS value#133]
...
📊 Tool 3: Cache impact on UDF performance:
   First access (cache miss): 0.149s
   Second access (cache hit): 0.023s
   Cache speedup: 6.6x
