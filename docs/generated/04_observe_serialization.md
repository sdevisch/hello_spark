...
+- Project [id#0L, cast((rand(1026605512160902240) * cast(100 as double)) as int) AS value#2, CASE WHEN ((id#0L % cast(2 as bigint)) = cast(0 as bigint)) THEN even ELSE odd END AS category#3]
...
Project [id#0L, (value#2 * 2) AS doubled#7]
+- Project [id#0L, cast((rand(1026605512160902240) * 100.0) as int) AS value#2]
   +- Range (0, 50000, step=1, splits=Some(11))

== Physical Plan ==
*(1) Project [id#0L, (value#2 * 2) AS doubled#7]
+- *(1) Project [id#0L, cast((rand(1026605512160902240) * 100.0) as int) AS value#2]
...
== Optimized Logical Plan ==
Aggregate [category#3], [category#3, avg(value#2) AS avg_value#14]
+- Project [cast((rand(1026605512160902240) * 100.0) as int) AS value#2, CASE WHEN ((id#0L % 2) = 0) THEN even ELSE odd END AS category#3]
   +- Range (0, 50000, step=1, splits=Some(11))
...
+- Exchange hashpartitioning(category#3, 200), ENSURE_REQUIREMENTS, [plan_id=29]
   +- *(1) HashAggregate(keys=[category#3], functions=[partial_avg(value#2)], output=[category#3, sum#19, count#20L])
      +- *(1) Project [cast((rand(1026605512160902240) * 100.0) as int) AS value#2, CASE WHEN ((id#0L % 2) = 0) THEN even ELSE odd END AS category#3]
         +- *(1) Range (0, 50000, step=1, splits=11)
...
+- Project [id#23L, cast((rand(-6275688357610166015) * cast(100 as double)) as int) AS value#25, CASE WHEN ((id#23L % cast(2 as bigint)) = cast(0 as bigint)) THEN even ELSE odd END AS category#26]
   +- Range (0, 50000, step=1, splits=Some(11))
...
== Optimized Logical Plan ==
Project [id#23L, (value#25 * 2) AS doubled#30]
+- Project [id#23L, cast((rand(-6275688357610166015) * 100.0) as int) AS value#25]
   +- Range (0, 50000, step=1, splits=Some(11))

== Physical Plan ==
*(1) Project [id#23L, (value#25 * 2) AS doubled#30]
+- *(1) Project [id#23L, cast((rand(-6275688357610166015) * 100.0) as int) AS value#25]
   +- *(1) Range (0, 50000, step=1, splits=11)
...
🚀 1. Native Spark function:
   ...
   ✅ Processed 100,000 rows in 0.775s
== Physical Plan ==
* Project (3)
+- * Project (2)
   +- * Range (1)
...
Output [2]: [id#38L, cast((rand(-1035376192638109264) * 100.0) as int) AS value#40]
Input [1]: [id#38L]
...
🐍 2. Python UDF:
   Execution plan with timing:
   ✅ Processed 100,000 rows in 0.057s
== Physical Plan ==
* Project (4)
+- BatchEvalPython (3)
   +- * Project (2)
...
+- Project [id#67L, value#69, category_python#75, (value#69 * value#69) AS value_squared#79]
   +- Project [id#67L, value#69, categorize_value(value#69)#74 AS category_python#75]
      +- Filter (value#69 > 10)
         +- Project [id#67L, cast((rand(1667611514151939788) * cast(100 as double)) as int) AS value#69, CASE WHEN ((id#67L % cast(2 as bigint)) = cast(0 as bigint)) THEN even ELSE odd END AS category#70]
...
+- Project [value#69, pythonUDF0#98 AS category_python#75, (value#69 * value#69) AS value_squared#79]
   +- BatchEvalPython [categorize_value(value#69)#74], [pythonUDF0#98]
      +- Filter (isnotnull(value#69) AND (value#69 > 10))
         +- Project [cast((rand(1667611514151939788) * 100.0) as int) AS value#69]
            +- Range (0, 80000, step=1, splits=Some(11))
      ...
      +- *(2) Project [value#69, pythonUDF0#98 AS category_python#75, (value#69 * value#69) AS value_squared#79]
         +- BatchEvalPython [categorize_value(value#69)#74], [pythonUDF0#98]
            +- *(1) Filter (isnotnull(value#69) AND (value#69 > 10))
               +- *(1) Project [cast((rand(1667611514151939788) * 100.0) as int) AS value#69]
                  +- *(1) Range (0, 80000, step=1, splits=11)


⏱️  Execution time: 1.176s

🔍 Results:
   Row(category_python='low', avg_value=17.534375282319992, avg_squared=323.7389104706839, count=11069)
   Row(category_python='high', avg_value=87.0010857227459, avg_squared=7621.792676306569, count=20263)
   Row(category_python='medium', avg_value=49.38930760727565, avg_squared=2648.0405989347805, count=39804)
...
✅ Processed 150,000 rows in 0.057s
...
+- Project [id#131L, cast((rand(703372960266908883) * 100.0) as int) AS value#133], Statistics(sizeInBytes=976.6 KiB)
...
+- *(1) Project [id#131L, cast((rand(703372960266908883) * 100.0) as int) AS value#133]
...
📊 Tool 3: Cache impact on UDF performance:
   First access (cache miss): 0.127s
